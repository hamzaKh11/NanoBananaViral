...
const PaymentWall = ({ onLogout, userId, onPlanUpdated }: { onLogout: () => void, userId: string, onPlanUpdated: () => void }) => {
  ...
  const handleBuy = (planKey: string) => {
    const variantId = LEMON_SQUEEZY_CONFIG.variantIds[planKey as keyof typeof LEMON_SQUEEZY_CONFIG.variantIds];
       ...
       <div className="max-w-5xl w-full grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          ...
          <div className="text-left space-y-6">
            ...
            <h1 className="text-4xl md:text-6xl font-black font-display text-gray-900 dark:text-white leading-tight">
              Unlock the <br/><span className="text-brand-yellow">Viral Engine.</span>
            </h1>
            <p className="text-lg text-gray-500 dark:text-gray-400">
              You are currently on the <strong>Free Tier</strong>. Upgrade to access our GPU cluster, 4K rendering, and AI Face Integration.
            </p>
            ...
            <div className="bg-white dark:bg-zinc-900 p-6 rounded-2xl border border-gray-200 dark:border-zinc-800 shadow-xl relative overflow-hidden">
               <div className="absolute top-0 right-0 bg-brand-yellow text-black text-[10px] font-bold px-3 py-1">POPULAR</div>
              ...
              <ul className="space-y-3 mb-6">
                 <li className="flex gap-2 text-sm"><Check className="text-brand-yellow"/> 100 Viral Credits</li>
                 <li className="flex gap-2 text-sm"><Check className="text-brand-yellow"/> Remove Watermarks</li>
                 <li className="flex gap-2 text-sm"><Check className="text-brand-yellow"/> Commercial License</li>
              </ul>
              <Button onClick={() => handleBuy('creator')} className="w-full py-4 text-lg font-bold shadow-lg bg-brand-yellow text-black hover:bg-yellow-400">
                Upgrade Now
              </Button>
              <p className="text-center text-xs text-gray-400 mt-3">Secure 256-bit SSL Payment via Lemon Squeezy</p>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center justify-center text-center space-y-6 opacity-90">
            ...
            <div>
                ...
                <p className="font-mono font-bold text-gray-700 dark:text-white">{userId}</p>
            </div>
            ...
            <button onClick={onLogout} className="text-xs font-bold text-gray-400 hover:text-red-500 flex items-center gap-1 transition">
...
const App: React.FC = () => {
  ...
  if (isLoading) return <div className="h-screen flex items-center justify-center bg-white dark:bg-black"><Loader2 className="animate-spin text-brand-yellow w-12 h-12"/></div>;
